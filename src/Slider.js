...
function Slider(prop){
    ...
    const rangeHasChanged = (event) =>{
        ...
        prop.getValue(event.target.value)
    }

    return(
        <div style={{margin:'20px'}}>
        <p style={{fontSize:'larger'}}>{prop.heading}</p>
        <p style={{fontSize:'x-large'}}>{prop.symbol} {prop.value}</p>
        <input type='range' style={{width:'45%'}} min={prop.min} max={prop.max} value={prop.value} onChange={rangeHasChanged}></input>
